...
- **여러 어절·문장·연결어** — 이 도구는 입력의 공백을 지우고 **한 덩어리로** 봅니다. 그래서 `한국어 및 한국문화`, `청군 대 백군`, `사과 배 귤 등`처럼 `및·등·겸·대·내지`로 이어지거나 열거하는 표현(제45항)과 문장 전체는 다루지 못합니다. 오히려 공백을 지워 `이사장 및 이사`를 `이사장및이사`로 잘못 붙이기도 하므로, **단어나 짧은 표현 하나씩** 넣어 주세요. 여러 어절·문장 분석은 추후 과제입니다.
